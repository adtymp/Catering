...
<div x-show="deleteAdmin" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        <div class="text-center border-b pb-4">
            <h2 class="text-xl font-semibold text-red-600">Konfirmasi Penghapusan</h2>
            <p class="mt-2 text-gray-700 text-sm">
                Masukkan password untuk menghapus <span class="font-bold text-black">{{ $admin->name }}</span>.
            </p>
        </div>

        <form method="POST" action="{{ route('admin.admin.delete', $admin->id) }}" class="mt-6 space-y-4">
            @csrf
            <div>

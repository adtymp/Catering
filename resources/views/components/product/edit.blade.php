...
<div x-show="editProduct" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 w-100 max-h-[90vh] overflow-y-auto">
        <div class="flex justify-end">
            <button @click="editProduct = false" class="text-gray-500 hover:text-red-500">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
            </button>
        </div>
        <form method="POST" action="{{ route('productroom.product.update', $product->id) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            <h3 class="text-xl text-center font-bold text-amber-400">Edit Produk</h3>
            <div class="flex flex-col flex-grow mb-3">
                <div x-data="{ productImage: [], removeFileImage(index) { this.productImage.splice(index, 1); } }"
                    ...
                    <template x-if="productImage.length === 0">
                        <div class="flex flex-col items-center justify-center py-10 text-center text-gray-600">
                            @if($product->image)
                            <div class="mb-4 w-80">
                                <img src="{{ asset('storage/' . $product->image) }}" alt="Current Image" class="w-screen h-32 object-cover rounded">
                            </div>
                            @endif
                        </div>
                    </template>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $product->name }}" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"">
                            </div>
                            <div>
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ $product->price }}" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                </div>
                <div>
                    <label class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300 text-left">Minimal Pembelian</label>
                    <input type="number" name="minPax" value="{{ $product->minPax }}" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                </div>
                <div>
                    <label class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300 text-left">Deskripsi</label>
                    <div class="mt-2">
                        <textarea name="deskripsi" rows="3" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">{{ trim($product->deskripsi) }}</textarea>
                    </div>
                </div>

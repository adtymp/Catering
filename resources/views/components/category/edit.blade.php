@props(['kategori'])
<div x-show="editCategory" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg w-80">
        <div class="flex justify-end">
            <button @click="editCategory = false" class="text-gray-500 hover:text-red-500">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                </svg>
            </button>
        </div>
        <form method="POST" enctype="multipart/form-data" action="{{ route('productroom.category.update', $kategori->id) }}" class="space-y-4">
            @csrf
            <h3 class="text-xl text-center font-bold text-amber-400">Edit Kategori</h3>
                ...
                <div
                    ...
                    <template x-if="files.length === 0">
                        <div class="flex flex-col items-center justify-center py-10 text-center text-gray-600">
                            @if($kategori->icon)
                            <div class="mb-4">
                                <img src="{{ asset('storage/' . $kategori->icon) }}" alt="Current Image" class="w-full h-32 object-cover rounded">
                            </div>
                            @endif
                        </div>
                    </template>
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ $kategori->name }}" class="w-full mt-1 p-2 border rounded-lg" required>
            </div>
